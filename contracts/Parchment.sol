...
contract Parchment {
     ...
     * _hash: 被授权数据哈希值
     */
    function cancelAuthzData(address _to, string calldata _hash)
        external
        dataExists(msg.sender, _hash)
        ownerOrAuthorized(_to, _hash)
    {
     ...
     * _hash: 数据哈希值
     */
    function listApplyRecord(string calldata _hash)
        external
        view
        returns (AuthzApplyInfo[] memory)
    {
